...
async def process_item(message: Message, state: FSMContext):
    ...
    await message.answer("Шаг 3/6: Пришлите фото")
...
async def process_username(message: Message, state: FSMContext):
    ...
    await message.answer("Шаг 6/6: Введите промокод (или «нет»)")
        ...
        try:
            ...
            async with session.post(GOOGLE_SCRIPT_URL, json=payload) as resp:
                ...
                if result.get("valid"):
                    ...
                    await message.answer(f"✅ Промокод активирован! Скидка: {discount}%")
                    await finish_order(message, state)
                else:
                    await message.answer("❌ Промокод недействителен")
        except:
            await message.answer("⚠️ Ошибка. Продолжим без промокода.")
            await state.update_data(promo_used=None, discount=0)
            await finish_order(message, state)
...
async def finish_order(message: Message, state: FSMContext):
    ...
    if data.get('promo_used'):
        text += f"\n🎟️ {data['promo_used']} (скидка {data['discount']}%)"
