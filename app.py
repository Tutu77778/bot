...
def main_keyboard(user_id: int):
    keyboard = [[KeyboardButton(text="📝 Заполнить заявку")], [KeyboardButton(text="👤 Мой профиль")]]
...
def get_status_emoji(status):
    return {"pending": "⏳", "paid": "💳", "completed": "✅", "cancelled": "❌"}.get(status, "📦")
...
async def show_profile(message: Message):
    ...
    await message.answer(
        ...
        f"⏳ В обработке: {status_counts['pending']}\n"
        f"💳 Оплачено: {status_counts['paid']}\n"
        f"✅ Выполнено: {status_counts['completed']}\n"
        f"❌ Отменено: {status_counts['cancelled']}",
...
async def process_game(message: Message, state: FSMContext):
    ...
    await message.answer("📦 Шаг 2/6: Название товара")
...
async def process_item(message: Message, state: FSMContext):
    ...
    await message.answer("🖼️ Шаг 3/6: Фото товара")

@dp.message(OrderForm.photo, F.photo)
async def process_photo(message: Message, state: FSMContext):
    await state.update_data(photo=message.photo[-1].file_id)
    await state.set_state(OrderForm.quantity)
    await message.answer("🔢 Шаг 4/6: Количество")
...
async def process_quantity(message: Message, state: FSMContext):
    ...
    await message.answer("👤 Шаг 5/6: Ваш Telegram username (без @)")
...
async def process_username(message: Message, state: FSMContext):
    ...
    await message.answer("🎟️ Шаг 6/6: Промокод (или «нет»)")
...
async def finish_order(message: Message, state: FSMContext):
    ...
    order = {
        ...
        "discount": data.get('discount', 0), "promo_used": data.get('promo_used'),
        "status": "pending", "date": datetime.now().isoformat(), "photo": data.get('photo')
    }
    orders = load_orders()
    orders.append(order)
    save_orders(orders)
    await send_order_to_group(order)
    await message.answer(f"✅ Заказ принят!\nНомер: #{order_id}", reply_markup=main_keyboard(message.from_user.id))
    await state.clear()
...
async def send_order_to_group(order):
    ...
    # КНОПКИ ОСТАЮТСЯ ДЛЯ ВСЕХ СТАТУСОВ КРОМЕ completed И cancelled
    keyboard = []
    ...
    if order.get('message_id'):
        try:
            await bot.edit_message_text(chat_id=ADMIN_GROUP_ID, message_id=order['message_id'], text=text, reply_markup=markup)
        except:
            pass
    else:
        if order.get('photo'):
...
async def process_order_action(callback: CallbackQuery):
    ...
    if not order:
        ...
        return
    status_map = {'cancel': ('cancelled', '❌ Ваш заказ отменён'), 'paid': ('paid', '💳 Ваш заказ оплачен'), 'complete': ('completed', '✅ Ваш заказ выполнен')}
    new_status, user_msg = status_map[action]
    update_order_status(order_id, new_status)
    order['status'] = new_status
    await send_order_to_group(order)
    try:
        await bot.send_message(order['user_id'], f"{user_msg}\n\nЗаказ #{order_id}")
    except:
        pass
...
async def cmd_start(message: Message):
    links_kb = InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="📱 Наш канал", url=CHANNEL_LINK)]])
